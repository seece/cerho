...
void main(void)
{
	...
	if(mod(coord.x+coord.y+iLocalTime*0.1,0.2)<0.1){
	...
	if(roundBox(coord, vec2(-0.3,0.0+vasenKulmakarva), vec2(0.08, 0.001) )<0.008)
		tulos.xyz-=vec3(0.7, 0.7, 0.7);
	tulos.xyz=tulos.xyz-vec3(min(max(-0.44+length(coord)*0.41,0.0),1.0))+vec3(0.06*rand(vec2(coord.x+coord.y,iLocalTime*coord.y*coord.x)));
